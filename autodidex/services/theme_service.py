...
class ThemeService:
    ...
    def __init__(self, base_path: Path):
        ...
        # Cache-first, then DB — fixes the original overwrite bug
    ...
    def toggle(self) -> str:
        """Advance to the next mode, persist to cache, and return the new mode name."""
        idx = self.CYCLE.index(self.current_mode)
        self.current_mode = self.CYCLE[(idx + 1) % len(self.CYCLE)]
        self._cache.set("theme", self.current_mode)
